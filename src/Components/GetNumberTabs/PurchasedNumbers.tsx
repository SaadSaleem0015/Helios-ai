...
import { FaSearch, FaPhone, FaUser, FaCalendar, FaLink, FaUnlink, FaTrash } from 'react-icons/fa';
...
const PurchasedNumbers: React.FC = () => {
  ...
  const fetchNumbers = async () => {
    ...
    try {
      ...
      const purchasedResponse = await backendRequest<PurchasedNumber[]>('GET', '/purchased_numbers');
      const purchased = Array.isArray(purchasedResponse)
  ...
  const handleDeleteSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await backendRequest('POST', '/remove-phone-number', {
        phone_number: selectedNumber.phone_number
      });
      notifyResponse(response);
                    ...
                    <div className="mt-4 flex gap-2">
                      {num.attached_assistant ? (
                        ...
                        <button
                          ...
                          className="flex-1 py-2 px-3 bg-primary text-white hover:bg-primary/90 rounded-lg text-sm font-medium transition-colors duration-200"
                        >
