...
export function TwilioCredentialsCard({ onStatusChange }: TwilioCredentialsCardProps) {
  ...
  const fetchCreds = async () => {
    setChecking(true);
    try {
      const res = await backendRequest<TwilioCredentials | { success: false; detail: string }>(
        "GET",
        "/credentials"
      );
      if ("account_sid" in res && res.account_sid) {
        setCreds({
          account_sid: res.account_sid || "",
          auth_token: "", // Never show token back
          address_sid: (res as TwilioCredentials).address_sid ?? "",
        });
        setConnected(true);
        onStatusChange?.(true);
      } else {
  ...
  const upsertCreds = async () => {
    ...
    try {
      ...
      const res = await backendRequest(method, "/credentials", payload);
      notifyResponse(res);
      if (res.success) {
  ...
  const deleteCreds = async () => {
    setLoading(true);
    try {
      const res = await backendRequest("DELETE", "/credentials");
      notifyResponse(res);
      if (res.success) {
        setCreds({ account_sid: "", auth_token: "", address_sid: "" });
        setConnected(false);
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-200">
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2 text-sm font-medium rounded-lg bg-primary text-white hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {loading ? (
