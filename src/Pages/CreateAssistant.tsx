      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4 py-3 w-full">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full justify-between md:w-auto items-center">
          {/* <div className="relative w-full sm:w-auto">
            <select
              className="w-full min-w-[160px] px-4 py-2 text-sm rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/30 outline-none appearance-none bg-white pr-10"
              onChange={(e) => setSelectedCategory(e.target.value)}
              value={selectedCategory}
              disabled={assistantId && !isDataLoaded} // Disable select while loading
            >
          ...
          <div className="flex flex-row justify-between w-full sm:w-auto gap-3">
            {assistantId ? (
              <>
                <button
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${isButtonsDisabled
              ...
              <>
                <button
                  className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${isSavingDraft
                    ? "bg-gray-100 text-gray-500 cursor-not-allowed"
                    : "text-red-600 hover:bg-red-50"
                    }`}
